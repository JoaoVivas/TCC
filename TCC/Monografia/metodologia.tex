...
\begin{equation}
    \label{eq:v_p}
    v_p = \sqrt{\frac{(v_i^2+v_f^2)}{2}+a*d}
\end{equation}
...
\begin{equation}
    ...
    \Delta des_i = \Delta v_i*\Delta t_i+ \frac{a_{segment}*\Delta t_i^2}{2} 
\end{equation}
...
\begin{multline}
    ...
    - \frac{c}{m}(\dot{x} - \dot{x_b}) - \frac{k}{m}(x-x_b) = \ddot{x} \\
    - \frac{c}{m} \dot{x} - \frac{k}{m} x + \frac{c}{m} \dot{x_b} + \frac{k}{m} x_b  = \ddot{x} \\
\end{multline}
...
\begin{equation}
    \label{eq:der_a}
    \begin{split}        
        a_n = 2\frac{((\frac{des_n-des_i}{dt})-v_i)}{dt} \\
        v_n = v_i+a_ndt \\
    \end{split}
\end{equation}
