function [c,ceq] = dbi_hargraves_dt_x3(x)
    global max_acc
    global u_base
    ...
    ceq=[des_x(1) des_y(1) des_xb(1) des_yb(1)];    
    c=[];
    
    for i = 1 : (length(dt)-1)
        [vel_x(i+1),vel_y(i+1),acc_x(i+1),acc_y(i+1)] = dot_const_acc(...
            des_x(i), des_y(i), des_x(i+1), des_y(i+1), vel_x(i), vel_y(i), dt(i));
    
        [vel_xb(i+1),vel_yb(i+1),acc_xb(i+1),acc_yb(i+1)] = dot_const_acc(...
            des_xb(i), des_yb(i), des_xb(i+1), des_yb(i+1), vel_xb(i), vel_yb(i), dt(i));
        ...
        delta = hargraves(x_i,u_i,x_n,u_n,dt(i),@dynamic_model);
